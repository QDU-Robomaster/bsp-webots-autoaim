...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--run-root', type=Path, default=Path('/home/xiao/runs'))
...
def main() -> int:
    ...
    run_dir = args.run_root / f'webots_preview_{utc_stamp()}'
    ...
    try:
        with webots_log.open('w', encoding='utf-8') as wf, controller_log.open('w', encoding='utf-8') as cf:
            while True:
                ...
                if controller_proc is not None and controller_proc.stdout is not None:
                    while True:
                        c_line = controller_proc.stdout.readline()
                        if c_line == '':
                            break
                        cf.write(c_line)
                        cf.flush()
                        print('controller: ' + c_line, end='')
